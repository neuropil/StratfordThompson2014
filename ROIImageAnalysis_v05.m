function varargout = ROIImageAnalysis_v05(varargin)
% ROIIMAGEANALYSIS_V05 MATLAB code for ROIImageAnalysis_v05.fig
%      ROIIMAGEANALYSIS_V05, by itself, creates a new ROIIMAGEANALYSIS_V05 or raises the existing
%      singleton*.
%
%      H = ROIIMAGEANALYSIS_V05 returns the handle to a new ROIIMAGEANALYSIS_V05 or the handle to
%      the existing singleton*.
%
%      ROIIMAGEANALYSIS_V05('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ROIIMAGEANALYSIS_V05.M with the given input arguments.
%
%      ROIIMAGEANALYSIS_V05('Property','Value',...) creates a new ROIIMAGEANALYSIS_V05 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ROIImageAnalysis_v05_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ROIImageAnalysis_v05_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ROIImageAnalysis_v05

% Last Modified by GUIDE v2.5 22-Jun-2014 14:47:49

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ROIImageAnalysis_v05_OpeningFcn, ...
    'gui_OutputFcn',  @ROIImageAnalysis_v05_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ROIImageAnalysis_v05 is made visible.
function ROIImageAnalysis_v05_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ROIImageAnalysis_v05 (see VARARGIN)

% Choose default command line output for ROIImageAnalysis_v05
...
% UIWAIT makes ROIImageAnalysis_v05 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ROIImageAnalysis_v05_OutputFcn(hObject, eventdata, handles)
...
for imI = 1:length(imageNames)
    ...
    % Densities of individual labels
    labeltwo(~polyMask) = 0;
...
switch exitquest
    case 'Yes'
        ...
        labeloneDen = zeros(length(handles.FileNames),1);
        labeltwoDen = zeros(length(handles.FileNames),1);
        for sei = 1:length(handles.FileNames)
            ...
            labeloneDen(sei) = handles.PixelDataOut.OD{1,sei}.labeloneArea;
            labeltwoDen(sei) = handles.PixelDataOut.OD{1,sei}.labeltwoArea;
        end
        
        DS = dataset(sectionNum, labelonePC, labeltwoPC, labeloneDen, labeltwoDen, 'VarNames',...
            {'Section',strcat(handles.PixelDataOut.OD{1,1}.labeloneName,'_PN'),...
            strcat(handles.PixelDataOut.OD{1,1}.labeltwoName,'_PN'),...
            strcat(handles.PixelDataOut.OD{1,1}.labeloneName,'_Den'),...
            strcat(handles.PixelDataOut.OD{1,1}.labeltwoName,'_Den')});
